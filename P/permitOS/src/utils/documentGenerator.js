...
Design parameters:
  ...
  NOx         | 0.0310        | Screening factor; confirm engine certification | ${fmt(results.genset.gensetNox)}
  CO          | 0.0091        | Screening factor; confirm engine certification | ${fmt(results.genset.gensetCO)}
  PM₁₀/PM₂.₅  | 0.0300        | Screening factor; confirm engine certification | ${fmt(results.genset.gensetPM)}
  SO₂         | 0.00205       | Screening factor based on assumed fuel sulfur  | ${fmt(results.genset.gensetSo2, 2)}
  VOC         | 0.0028        | Screening factor; confirm engine certification | ${fmt(results.genset.gensetVoc, 2)}
...
NOx       | ${fmt(controlled.nox - results.genset.gensetNox)} | ${fmt(results.genset.gensetNox)} | ${fmt(controlled.nox)} | 100 tpy | ${controlled.nox >= 100 ? 'YES — PSD Review Required' : 'NO — Below Major Source Threshold'}
CO        | ${fmt(controlled.co - results.genset.gensetCO)} | ${fmt(results.genset.gensetCO)} | ${fmt(controlled.co)} | 100 tpy | ${controlled.co >= 100 ? 'YES — PSD Review' : 'NO'}
...
Condition A-2 (NOx Emission Rate Limit): The NOx emission rate for each CTG shall not exceed ${noxFactor.toFixed(4)} lb/MMBtu. Any ppmvd equivalent and oxygen correction basis must be confirmed against the selected turbine's enforceable permit limit and test method. Compliance shall be demonstrated through an initial performance stack test on the schedule required by the final permit.

Condition A-3 (CO Limit): The CO emission rate for each CTG shall not exceed ${coFactor.toFixed(4)} lb/MMBtu. The final permit condition must use the units, averaging period, and oxygen basis accepted by the permitting authority. Oxidation catalyst pressure differential and exhaust temperature should be evaluated as possible surrogate parameters.
...
These conditions, when incorporated into the preconstruction permit and issued under the state SIP-approved NSR program (or 40 CFR § 52.21 for federal delegation), create federally enforceable limitations that establish the controlled PTE as the basis for major source / synthetic minor / minor source classification under NSR, Title V, and applicable NSPS/NESHAP programs.`
  ...
  B. Calculation Verification: All PTE calculations are performed using the standard formula:
     ...
     Each calculation must be independently reviewed for unit conversion accuracy (MMBtu → MMBtu, lb → tons), source data, and methodology consistency before filing.
...
RBLC Database: Any precedent entries shown in this draft are illustrative screening references only. A qualified reviewer must run and document a current EPA RBLC search before relying on this analysis. Comparable facilities should be selected based on source type, size, fuel, duty cycle, and permitting context.`
      },
      {
