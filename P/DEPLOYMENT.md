...
- Optional production database: `DATABASE_URL` (runs the Postgres schema initializer)
- Optional S3-compatible storage: `S3_BUCKET`, region/endpoint, and credentials
- Optional file inspection: `CLAMSCAN_EXECUTABLE`, `PDFTOTEXT_EXECUTABLE`
- Optional EPA model execution: `AERMOD_EXECUTABLE`
...
The current application retains SQLite compatibility for pilot deployments and includes the Postgres production schema. To migrate existing core records:

```bash
cd permitOS/server
DATABASE_URL="..." DB_PATH="/path/to/permitos.db" pnpm migrate:postgres
```

Run the migration against a staging database first, verify record counts and tenant isolation, then cut over application repositories in a controlled release.

## Object storage

Without `S3_BUCKET`, evidence is stored privately on the mounted filesystem and downloaded through short-lived HMAC-signed links. With S3-compatible configuration, PermitOS uploads encrypted objects and returns provider-signed download URLs.

Production deployments should configure malware scanning and extraction workers. An upload with scanning unavailable is explicitly marked `not-configured`, never silently marked clean.
...
- Approval-chain validation passes and immutable approval triggers are present.
- Evidence objects have hashes, scan status, extraction status, and expiring download links.
