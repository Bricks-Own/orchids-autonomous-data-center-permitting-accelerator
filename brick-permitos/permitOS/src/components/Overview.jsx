...
const tools = [
  {
    icon: '🔍',
    name: 'Regulatory Search Assistant',
  ...
  {
    icon: '🤖',
    name: 'Regulator Response Assistant',
...
const COMPETITOR_COMPARE = [
  {
    ...
    brick: { value: '< 1 hour', color: 'text-green-400' },
    traditional: { value: '3–5 weeks', color: 'text-red-400' },
    scout: { value: 'Days–weeks', color: 'text-amber-400' },
  },
  {
    feature: 'PTE workbook generation',
    brick: { value: 'Instant (AI)', color: 'text-green-400' },
    traditional: { value: '2–4 weeks', color: 'text-red-400' },
    scout: { value: 'Semi-automated', color: 'text-amber-400' },
  },
  {
    feature: 'BACT analysis draft',
    brick: { value: '1–2 days (AI)', color: 'text-green-400' },
    traditional: { value: '4–8 weeks', color: 'text-red-400' },
    scout: { value: 'Not included', color: 'text-gray-600' },
  },
  {
    feature: 'AERMOD input file builder',
    brick: { value: 'Auto-generated', color: 'text-green-400' },
    traditional: { value: '4–6 weeks', color: 'text-red-400' },
    scout: { value: 'Not included', color: 'text-gray-600' },
  },
  {
    feature: 'Water permits (NPDES/SPCC/316b)',
    brick: { value: 'Full AI package', color: 'text-green-400' },
    traditional: { value: '6–10 weeks', color: 'text-red-400' },
  ...
  },
  {
    ...
    brick: { value: 'Live controls + logs', color: 'text-green-400' },
    traditional: { value: 'Not included', color: 'text-gray-600' },
    scout: { value: 'Dashboard only', color: 'text-amber-400' },
  },
  {
    feature: 'Permit-linked digital twin',
    brick: { value: 'Hour-by-hour sim', color: 'text-green-400' },
    traditional: { value: 'Not included', color: 'text-gray-600' },
    scout: { value: 'Not included', color: 'text-gray-600' },
  },
  {
    feature: 'RAI / deficiency response',
    brick: { value: '< 24 hrs (AI)', color: 'text-green-400' },
    traditional: { value: '3–6 weeks', color: 'text-red-400' },
    scout: { value: 'Not included', color: 'text-gray-600' },
  },
  {
    feature: 'Reduces permit risk via controls',
    brick: { value: 'Yes — lowers PTE', color: 'text-green-400' },
    traditional: { value: 'No', color: 'text-gray-600' },
    scout: { value: 'No', color: 'text-gray-600' },
  },
];
...
export default function Overview({ setActiveTab }) {
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Hero */}
      <div className="rounded-2xl bg-gradient-to-br from-indigo-950 via-gray-900 to-gray-950 border border-indigo-800/40 p-8">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <p className="text-gray-400 text-sm leading-relaxed">
              A unified workspace for data-center permitting—from early site screening through draft permit packages and operational compliance planning.
              PermitOS combines regulatory decision support, emissions calculations, water-pathway analysis, and document automation in one reviewable record.
            </p>
          </div>
          <div className="grid grid-cols-3 gap-3 min-w-[320px]">
            {metrics.map(m => (
              <div key={m.label} className="bg-gray-900/60 border border-gray-700/40 rounded-xl p-3 text-center">
                <div className="text-2xl font-bold text-indigo-400">{m.value}</div>
                <div className="text-xs text-gray-300 mt-0.5 font-medium">{m.label}</div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="rounded-xl border border-gray-700/40 bg-gray-900/40 p-5">
          <h3 className="text-sm font-semibold text-gray-300 mb-3">Why BigWatt Needs This Now</h3>
        ...
        <div className="rounded-xl border border-indigo-700/30 bg-indigo-950/20 p-5">
          <h3 className="text-sm font-semibold text-indigo-300 mb-3">Brick's Unfair Advantage Over Consultants</h3>
          <ul className="space-y-2">
            {[
      ...
      {/* Tool Suite */}
      <div>
        <h2 className="text-sm font-semibold text-gray-300 mb-4">Full Tool Suite</h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {tools.map(t => (
            <div key={t.name} className={`rounded-xl border p-5 ${t.color}`}>
              ...
              <div className="bg-black/20 rounded-lg p-2.5 text-xs text-gray-500 italic">
                <span className="text-indigo-400 not-italic font-medium">vs. traditional: </span>
                {t.vs}
              </div>
            </div>
          ))}
      ...
      <div className="rounded-xl border border-gray-700/40 bg-gray-900/40 p-5">
        <h3 className="text-sm font-semibold text-gray-300 mb-4">Regulatory Foundation — Federal Citations</h3>
        <div className="grid md:grid-cols-3 gap-4 text-xs">
          <div>
            <p className="text-indigo-400 font-semibold mb-2">Clean Air Act / Air Rules</p>
          ...
          <div>
            <p className="text-green-400 font-semibold mb-2">Other Regulatory Requirements</p>
            <ul className="space-y-1 text-gray-500">
      ...
      {/* Workflow Steps */}
      <div>
        <h2 className="text-sm font-semibold text-gray-300 mb-4">How It Works — 6-Step AI Workflow</h2>
        <div className="grid md:grid-cols-3 lg:grid-cols-6 gap-3">
          {WORKFLOW_STEPS.map((s, i) => (
            <div key={s.step} className={`rounded-xl border p-4 relative ${s.color}`}>
              {i < WORKFLOW_STEPS.length - 1 && (
                <div className="hidden lg:block absolute -right-3 top-1/2 -translate-y-1/2 z-10 text-gray-700 text-lg">→</div>
              )}
              <div className="text-2xl mb-2">{s.icon}</div>
              <div className="text-xs text-gray-500 font-mono mb-1">STEP {s.step}</div>
              <div className="text-sm font-semibold text-white mb-1">{s.label}</div>
              <div className="text-xs text-gray-500 leading-relaxed mb-2">{s.desc}</div>
              <div className="text-xs bg-black/20 rounded-full px-2 py-0.5 inline-block text-indigo-300 font-medium">{s.time}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-700/40 bg-gray-900/40 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700/40">
          <h3 className="text-sm font-semibold text-gray-300">Brick PermitOS vs. Traditional Consulting vs. Scout AI</h3>
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="bg-gray-800/60">
                  <th className="px-4 py-3 text-left text-gray-400 font-semibold">Feature</th>
                  <th className="px-4 py-3 text-left text-green-400 font-semibold">Brick PermitOS</th>
                  <th className="px-4 py-3 text-left text-red-400 font-semibold">Traditional Consulting (ASG-style)</th>
                  <th className="px-4 py-3 text-left text-amber-400 font-semibold">Scout AI (Sitetracker)</th>
                </tr>
              </thead>
              <tbody>
                {COMPETITOR_COMPARE.map((row, i) => (
                  <tr key={row.feature} className={`border-t border-gray-800/40 ${i % 2 === 0 ? 'bg-gray-900/20' : ''}`}>
        ...
        {!showCompare && (
          <div className="px-5 py-3 flex items-center gap-6 text-xs">
            <span className="text-green-400 font-semibold">Brick: integrated permitting workflow</span>
            <span className="text-amber-400">Software tools: capability varies by vendor</span>
            <span className="text-gray-500">Consulting: scope and delivery vary by engagement</span>
          </div>
        )}
      </div>

      {/* Start CTA */}
      <div className="rounded-xl border border-indigo-700/30 bg-gradient-to-r from-indigo-950/60 to-violet-950/40 p-6 text-center">
        ...
        {setActiveTab && (
          <button onClick={() => setActiveTab('intake')}
            className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-xl text-sm font-semibold transition-all shadow-lg shadow-indigo-900/40">
