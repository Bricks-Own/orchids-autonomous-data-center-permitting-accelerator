...
function DocRow({ doc, docType, generated, compliance, onPreview, onGenerate }) {
  ...
  const statusIcon = compObj.status === 'pass' ? 'text-blue-300' : compObj.status === 'fail' ? 'text-red-400' : 'text-yellow-400';
  const statusBg = compObj.status === 'pass' ? 'bg-blue-900/20 border-blue-700/40' : compObj.status === 'fail' ? 'bg-red-900/20 border-red-700/40' : 'bg-yellow-900/20 border-yellow-700/40';
  const statusLabel = compObj.status === 'pass' ? 'Draft check complete' : compObj.label;
              ...
              {showSource && validationInfo && (
                <div className="absolute z-50 top-full left-0 mt-1 w-72 bg-gray-800 border border-gray-600 rounded-xl p-3 shadow-2xl">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-gray-400 font-medium">Reference structure compared</span>
                  </div>
                  <p className="text-xs text-gray-300 leading-relaxed mb-1.5">
                    The report structure has been compared with example consulting
                    deliverables. This does not validate calculations, assumptions,
                    regulatory applicability, or filing readiness.
                  </p>
                  {validationInfo.validatedSections.length > 0 && (
                    <div className="bg-gray-900/60 rounded-lg px-2 py-1.5 border border-gray-700/40 mb-1.5">
                      <span className="text-[10px] text-gray-500 uppercase tracking-wider">Sections Compared</span>
        ...
        <>
          ...
          <div className="rounded-xl border border-amber-800/40 bg-amber-950/10 p-4">
            ...
            <p className="text-xs text-gray-500 leading-relaxed">
              Documents are unverified working drafts generated from user inputs, deterministic calculations, and template content. Citations, factors, assumptions, and applicability may be incomplete or outdated.
              A qualified permitting professional and, where required, a licensed Professional Engineer (PE) must independently verify and certify each document before submission.
