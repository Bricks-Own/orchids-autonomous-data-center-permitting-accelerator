...
export default function RegulatorCopilot({ results, inputs }) {
  ...
  const [chatHistory, setChatHistory] = useState([
    {
      role: 'assistant',
      content: `Welcome to the Brick Regulator Response Assistant. I can search the available regulatory corpus and help draft a response for ${inputs?.siteName || 'the current site'}. Every draft must be checked against the actual permit record, source documents, modeling files, and current agency requirements before use.`,
    }
  ]);
  ...
  const sendChat = async () => {
    ...
    try {
      ...
      const modeNotice = response.type === 'rag'
        ? 'Regulatory-search-only mode (Claude drafting is unavailable).\n\n'
        : 'Claude-assisted draft with regulatory search context. Unverified.\n\n';
      const answerText = response.content || response.answer || response.response || '';
      setChatHistory(prev => [...prev, { role: 'assistant', content: modeNotice + answerText }]);
    } catch {
      ...
      responseText = 'The connected regulatory assistant is unavailable. No substitute factual response was generated. Use demo templates only as illustrative outlines and verify every statement against project evidence.';
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="rounded-xl border border-violet-700/30 bg-violet-950/20 p-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-base font-semibold text-white">Regulator Response Assistant</h2>
              ...
              <div key={rai.id} className="rounded-xl border border-gray-700/40 bg-gray-900/40 overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-xs text-amber-300 bg-amber-900/30 border border-amber-700/40 rounded-full px-2.5 py-0.5">Illustrative draft</span>
      ...
      {activeMode === 'chat' && (
        <div className="rounded-xl border border-gray-700/40 bg-gray-900/40 flex flex-col" style={{ height: '560px' }}>
          <div className="flex items-center gap-3 p-4 border-b border-gray-800/40">
            ...
            <div>
              <div className="text-sm font-medium text-gray-300">Brick Regulator Response Assistant</div>
